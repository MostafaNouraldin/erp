...
-- This file is auto-generated based on the Drizzle schema.
-- It is recommended to run this file to initialize a new tenant database.
...
-- =================================================================
-- SECTION 1: MAIN SCHEMA TABLES (System Administration & Core)
-- These tables manage the SaaS aspect of the application.
-- =================================================================

-- Stores information about each tenant (company).
...
-- Stores all users for all tenants. The `tenant_id` links a user to their company.
CREATE TABLE "users" (
	"id" varchar(256) PRIMARY KEY NOT NULL,
	"name" varchar(256) NOT NULL,
	"email" varchar(256) NOT NULL UNIQUE,
...
-- Maps tenants to the modules they are subscribed to.
...
-- Stores renewal/upgrade requests from existing tenants.
...
-- Stores tenant-specific settings as a JSON object.
CREATE TABLE "company_settings" (
	"id" varchar(256) PRIMARY KEY NOT NULL, -- This is the tenantId
...
-- These tables exist within each tenant's schema.
-- =================================================================
...
-- Added at the end to avoid order-of-creation issues.
-- =================================================================
...
END $$;
DO $$ BEGIN
 ALTER TABLE "subscription_renewal_requests" ADD CONSTRAINT "subscription_renewal_requests_tenant_id_tenants_id_fk" FOREIGN KEY ("tenant_id") REFERENCES "tenants"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN
 ALTER TABLE "subscription_renewal_requests" ADD CONSTRAINT "subscription_renewal_requests_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
DO $$ BEGIN
...
-- This data should be inserted into a new tenant's database upon creation

...
import type { TransferFormValues, Product, Warehouse as TWarehouse } from './actions';
import { addTransfer, updateTransferStatus, deleteTransfer } from './actions';
...
export default function InventoryTransfersClient({ initialData }: ClientComponentProps) {
  ...
  const handleUpdateStatus = async (id: string, status: TransferFormValues['status']) => {
    try {
              ...
              <TableBody>
                {transfers.map((transfer) => (
                  <TableRow key={transfer.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>{new Date(transfer.date).toLocaleDateString('ar-SA')}</TableCell>
                    <TableCell>{warehouses.find(wh => wh.id === transfer.fromWarehouseId)?.name || transfer.fromWarehouseId}</TableCell>
                    <TableCell>{warehouses.find(wh => wh.id === transfer.toWarehouseId)?.name || transfer.toWarehouseId}</TableCell>
                    <TableCell>{products.find(i => i.id === transfer.productId)?.name || transfer.productId}</TableCell>

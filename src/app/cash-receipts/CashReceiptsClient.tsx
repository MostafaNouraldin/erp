...
import { PlusCircle, Edit, Trash2, Search, Printer, CheckCircle, Undo, Filter, ArrowUpSquare } from "lucide-react";
...
import { addCashReceipt, updateCashReceipt, deleteCashReceipt, updateCashReceiptStatus } from './actions';
import type { CashReceiptFormValues, CashAccount, RevenueAccount, Customer } from './actions';
...
interface ClientComponentProps {
    initialData: {
        cashReceipts: CashReceiptFormValues[];
...
export default function CashReceiptsClient({ initialData }: ClientComponentProps) {
  const [cashReceipts, setCashReceipts] = useState(initialData.cashReceipts);
  const [cashAccounts, setCashAccounts] = useState(initialData.cashAccounts);
  ...
  const [receiptToEdit, setReceiptToEdit] = useState<CashReceiptFormValues | null>(null);
  const [showPrintReceiptDialog, setShowPrintReceiptDialog] = useState(false);
  const [selectedReceiptForPrint, setSelectedReceiptForPrint] = useState<CashReceiptFormValues | null>(null);
  const { formatCurrency } = useCurrency();
  const { toast } = useToast();
  
  useEffect(() => {
    setCashReceipts(initialData.cashReceipts);
  ...
  useEffect(() => {
    if (showManageReceiptDialog) {
        if (receiptToEdit) {
            ...
            form.reset({ date: new Date(), cashAccountId: "", revenueAccountId: "", payerName: "", description: "", amount: 0, referenceNumber: "", status: "مسودة", customerId: "" });
        }
    }
  }, [receiptToEdit, form, showManageReceiptDialog]);

  const handleReceiptSubmit = async (values: CashReceiptFormValues) => {
    try {
      if (receiptToEdit) {
        await updateCashReceipt({ ...values, id: receiptToEdit.id! });
        toast({ title: "تم التعديل", description: "تم تعديل المقبوضات النقدية بنجاح." });
      } else {
        await addCashReceipt(values);
        toast({ title: "تم الإنشاء", description: "تم إنشاء مقبوضات نقدية جديدة." });
      }
  ...
  const handleDeleteReceiptAction = async (receiptId: string) => {
    try {
        await deleteCashReceipt(receiptId);
  ...
  const handlePostReceiptAction = async (receiptId: string) => {
    try {
        await updateCashReceiptStatus(receiptId, "مرحل");
  ...
  const handleUnpostReceiptAction = async (receiptId: string) => {
    try {
        await updateCashReceiptStatus(receiptId, "مسودة");
  ...
  const handlePrintReceipt = (receipt: CashReceiptFormValues) => {
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center text-2xl md:text-3xl">
            <ArrowUpSquare className="me-2 h-8 w-8 text-primary" />
            المقبوضات النقدية
          </CardTitle>
          <CardDescription>
            تسجيل وتتبع جميع المبالغ المستلمة نقداً في خزائن الشركة.
          </CardDescription>
        </CardHeader>
      </Card>

      <div className="my-6 flex justify-between items-center">
        <h2 className="text-xl font-semibold">سجل المقبوضات النقدية</h2>
        <Dialog open={showManageReceiptDialog} onOpenChange={(isOpen) => { setShowManageReceiptDialog(isOpen); if (!isOpen) setReceiptToEdit(null); }}>
          <DialogTrigger asChild>
            <Button className="shadow-md hover:shadow-lg transition-shadow" onClick={() => { setReceiptToEdit(null); form.reset(); setShowManageReceiptDialog(true); }}>
              <PlusCircle className="me-2 h-4 w-4" /> تسجيل مقبوضات نقدية
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-lg" dir="rtl">
            <DialogHeader>
              <DialogTitle>{receiptToEdit ? 'تعديل مقبوضات نقدية' : 'تسجيل مقبوضات نقدية جديدة'}</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleReceiptSubmit)} className="space-y-4 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormItem><FormLabel>حساب الخزينة (المستلم فيه)</FormLabel>
                        <Select onValueChange={field.onChange} defaultValue={field.value} dir="rtl">
                        <FormControl><SelectTrigger className="bg-background"><SelectValue placeholder="اختر حساب الخزينة" /></SelectTrigger></FormControl>
                        <SelectContent>{cashAccounts.map(acc => <SelectItem key={acc.id} value={acc.id}>{acc.name}</SelectItem>)}</SelectContent>
                        </Select><FormMessage /></FormItem>
                    )} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormField control={form.control} name="referenceNumber" render={({ field }) => (
                        <FormItem><FormLabel>الرقم المرجعي (اختياري)</FormLabel><FormControl><Input placeholder="رقم إيصال يدوي، إلخ" {...field} className="bg-background" /></FormControl><FormMessage /></FormItem>
                    )} />
                </div>
                <DialogFooter>
              ...
              <TableBody>
                {cashReceipts.map((receipt) => (
                  <TableRow key={receipt.id} className="hover:bg-muted/50">
                    ...
                    <TableCell className="font-medium">{cashAccounts.find(b => b.id === receipt.cashAccountId)?.name}</TableCell>
                          ...
                          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-accent" title="تعديل" onClick={() => { setReceiptToEdit(receipt); setShowManageReceiptDialog(true); }}>
                           ...
                           <AlertDialog>
                            ...
                            <AlertDialogContent dir="rtl">
                              <AlertDialogHeader><AlertDialogTitle>هل أنت متأكد من الحذف؟</AlertDialogTitle><AlertDialogDescription>سيتم حذف المقبوضات النقدية من "{receipt.payerName}" نهائياً.</AlertDialogDescription></AlertDialogHeader>
      ...
      <Dialog open={showPrintReceiptDialog} onOpenChange={setShowPrintReceiptDialog}>
        <DialogContent className="sm:max-w-3xl print-hidden" dir="rtl">
          <DialogHeader className="print-hidden">
            <DialogTitle>طباعة إيصال مقبوضات نقدية: {selectedReceiptForPrint?.id}</DialogTitle>
          </DialogHeader>
            ...
            <div className="printable-area bg-background text-foreground font-cairo text-sm p-4" data-ai-hint="receipt voucher">
              <div className="flex justify-between items-start pb-4 mb-6 border-b border-gray-300">
                ...
                <div className="text-left">
                  <h3 className="text-md font-semibold">إيصال مقبوضات نقدية</h3>
                  <p className="text-xs">Cash Receipt Voucher</p>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-6 text-xs">
                <div><strong>حساب الخزينة (المستلم فيه):</strong> {cashAccounts.find(b => b.id === selectedReceiptForPrint.cashAccountId)?.name}</div>

...
import { addCashExpense, updateCashExpense, deleteCashExpense, updateCashExpenseStatus } from './actions';
import type { CashExpenseFormValues, CashAccount, ExpenseAccount } from './actions';
...
interface ClientComponentProps {
    initialData: {
        cashExpenses: CashExpenseFormValues[];
        cashAccounts: CashAccount[];
        expenseAccounts: ExpenseAccount[];
    }
}

export default function CashExpensesClient({ initialData }: ClientComponentProps) {
  const [cashExpenses, setCashExpenses] = useState(initialData.cashExpenses);
  ...
  const [selectedExpenseForPrint, setSelectedExpenseForPrint] = useState<CashExpenseFormValues | null>(null);
  const { formatCurrency } = useCurrency();
  const { toast } = useToast();
  
  useEffect(() => {
    setCashExpenses(initialData.cashExpenses);
  ...
  const handlePostExpenseAction = async (expenseId: string) => {
    try {
        await updateCashExpenseStatus(expenseId, "مرحل");
  ...
  const handleUnpostExpenseAction = async (expenseId: string) => {
    try {
        await updateCashExpenseStatus(expenseId, "مسودة");
  ...
  const handlePrintExpense = (expense: CashExpenseFormValues) => {
              ...
              <TableBody>
                {cashExpenses.map((expense) => (
                  <TableRow key={expense.id} className="hover:bg-muted/50">
                    ...
                    <TableCell className="font-medium">{cashAccounts.find(b => b.id === expense.cashAccountId)?.name}</TableCell>
          ...
          {selectedExpenseForPrint && (
            <div className="printable-area bg-background text-foreground font-cairo text-sm p-4" data-ai-hint="voucher layout">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-6 text-xs">
                <div><strong>حساب الخزينة:</strong> {cashAccounts.find(c => c.id === selectedExpenseForPrint.cashAccountId)?.name}</div>

...
import { PlusCircle, Edit, Trash2, Search, UserCheck, FileText, Filter, Undo, UploadCloud } from "lucide-react";
...
import { addSettlement, updateSettlement, deleteSettlement, updateSettlementStatus, postSettlementToGL } from './actions';
import type { Employee, Settlement, SettlementAccount } from './actions';
...
const settlementSchema = z.object({
  ...
  accountId: z.string().min(1, "حساب التسوية مطلوب"),
...
export default function EmployeeSettlementsClientComponent({ initialData }: ClientComponentProps) {
  ...
  const [settlementAccounts, setSettlementAccounts] = useState(initialData.settlementAccounts);
  ...
  const [selectedSettlementForView, setSelectedSettlementForView] = useState<Settlement | null>(null);
  const { toast } = useToast();
  const { formatCurrency } = useCurrency();

  useEffect(() => {
    setSettlements(initialData.settlements);
    setEmployees(initialData.employees);
    setSettlementAccounts(initialData.settlementAccounts);
  }, [initialData]);
  ...
  useEffect(() => {
    if (settlementToEdit) {
      form.reset({
        ...settlementToEdit,
        date: new Date(settlementToEdit.date),
      });
    } else {
  ...
  const handleSettlementSubmit = async (values: SettlementFormValues) => {
    try {
        if (settlementToEdit) {
          await updateSettlement({ ...values, id: settlementToEdit.id! });
          toast({ title: "تم التعديل", description: "تم تعديل التسوية بنجاح." });
        } else {
          await addSettlement(values);
          toast({ title: "تم الإنشاء", description: "تم إنشاء التسوية بنجاح." });
        }
        setShowManageSettlementDialog(false);
        setSettlementToEdit(null);
    } catch (error) {
        toast({ title: "خطأ", description: "لم يتم حفظ التسوية.", variant: "destructive" });
    }
  };
  ...
  const handleDeleteSettlement = async (settlementId: string) => {
    try {
        ...
        toast({ title: "نجاح", description: "تم حذف التسوية بنجاح.", variant: "destructive" });
    } catch (error: any) {
  ...
  const handleApproveSettlement = async (settlementId: string) => {
    try {
        await updateSettlementStatus(settlementId, "معتمدة");
        toast({ title: "نجاح", description: `تم اعتماد التسوية رقم ${settlementId}.` });
    } catch (error: any) {
        toast({ title: "خطأ", description: error.message, variant: "destructive" });
    }
  };
  ...
  const handleCancelSettlement = async (settlementId: string) => {
    try {
        await updateSettlementStatus(settlementId, "ملغاة");
        toast({ title: "نجاح", description: `تم إلغاء التسوية رقم ${settlementId}.`, variant: "destructive" });
    } catch (error: any) {
        toast({ title: "خطأ", description: error.message, variant: "destructive" });
    }
  };
  ...
  const handlePostToGLAction = async (settlement: Settlement) => {
      try {
          await postSettlementToGL(settlement);
          toast({
            title: "تم طلب الترحيل للحسابات العامة",
            description: `سيتم ترحيل قيد تسوية الموظف رقم ${settlement.id}.`,
            variant: "default",
        });
      } catch (error: any) {
           toast({
              ...
              <form onSubmit={form.handleSubmit(handleSettlementSubmit)} className="space-y-4 py-4">
                ...
                <FormField control={form.control} name="accountId" render={({ field }) => (
                    <FormItem><FormLabel>حساب التسوية</FormLabel>
                        ...
                        <SelectContent>{settlementAccounts.map(acc => <SelectItem key={acc.id} value={acc.id}>{acc.name} ({acc.id})</SelectItem>)}</SelectContent>
                        </Select><FormMessage /></FormItem>
                    )} />
              ...
              <TableBody>
                {settlements.map((set) => (
                  <TableRow key={set.id} className="hover:bg-muted/50">
                    <TableCell>{new Date(set.date).toLocaleDateString('ar-SA', { calendar: 'gregory' })}</TableCell>
                    <TableCell className="font-medium">{employees.find(e => e.id === set.employeeId)?.name}</TableCell>
                    ...
                    <TableCell className="text-center space-x-1 rtl:space-x-reverse">
                      ...
                      {set.status === "مسودة" && (
                        <>
                          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-accent" title="تعديل" onClick={() => { setSettlementToEdit(set as SettlementFormValues); setShowManageSettlementDialog(true); }}>
                           ...
                           <AlertDialog>
                            ...
                            <AlertDialogContent dir="rtl">
                              <AlertDialogHeader><AlertDialogTitle>هل أنت متأكد من الحذف؟</AlertDialogTitle><AlertDialogDescriptionComponent>سيتم حذف التسوية للموظف "{employees.find(e => e.id === set.employeeId)?.name}" نهائياً.</AlertDialogDescriptionComponent></AlertDialogHeader>
                         ...
                         <AlertDialog>
                            ...
                            <AlertDialogContent dir="rtl">
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescriptionComponent>هل أنت متأكد من ترحيل التسوية رقم "{set.id}" للحسابات العامة؟</AlertDialogDescriptionComponent>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                    <AlertDialogCancel>إلغاء</AlertDialogCancel>
                                    <AlertDialogAction onClick={() => handlePostToGLAction(set)}>تأكيد الترحيل</AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
                         </AlertDialog>
                      )}
        ...
        <DialogContent className="sm:max-w-lg" dir="rtl">
          ...
          {selectedSettlementForView && (
            <div className="py-4 space-y-3">
              ...
              <div><strong>تاريخ التسوية:</strong> {new Date(selectedSettlementForView.date).toLocaleDateString('ar-SA', { calendar: 'gregory' })}</div>
              <div><strong>الموظف:</strong> {employees.find(e => e.id === selectedSettlementForView.employeeId)?.name}</div>
              <div><strong>نوع التسوية:</strong> {selectedSettlementForView.settlementType}</div>
              <div><strong>حساب التسوية:</strong> {settlementAccounts.find(a => a.id === selectedSettlementForView.accountId)?.name}</div>

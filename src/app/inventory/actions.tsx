...
import { products, categories, warehouses, stockRequisitions, stockRequisitionItems, stockIssueVouchers, stockIssueVoucherItems, stocktakes, goodsReceivedNotes, goodsReceivedNoteItems } from '@/db/schema';
...
export async function addGoodsReceivedNote(values: GoodsReceivedNoteFormValues) {
    ...
    await db.transaction(async (tx) => {
        await tx.insert(goodsReceivedNotes).values({ ...values, id: newId, status: 'مرحل للمخزون' }); // Assuming it's posted on creation for simplicity
        if (values.items.length > 0) {

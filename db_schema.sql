-- Comprehensive schema for the Al-Mustaqbal ERP system.
-- This script is designed to be idempotent (can be run multiple times without causing errors).

-- Ensure the 'main' schema exists for system-wide tables.
CREATE SCHEMA IF NOT EXISTS main;

-- ====================================================================
-- MAIN SCHEMA - System Administration, Tenants, Subscriptions
-- These tables reside in the main database, not in tenant-specific databases.
-- ====================================================================

-- Stores information about each tenant (company) using the system.
...
-- Stores system-level roles, primarily for Super Admins.
...
-- Stores system-level users, such as Super Admins.
...
-- Tracks which modules each tenant is subscribed to.
...
-- Stores new subscription requests from potential customers.
...
-- These tables should exist in each tenant's dedicated database.
-- ====================================================================

-- Roles within a specific tenant's organization.
...
-- Chart of Accounts for the tenant's accounting system.
...
CREATE TABLE IF NOT EXISTS stock_issue_voucher_items (
    id SERIAL PRIMARY KEY,
    issue_voucher_id VARCHAR(256) NOT NULL REFERENCES stock_issue_vouchers(id) ON DELETE CASCADE,
    product_id VARCHAR(256) NOT NULL REFERENCES products(id),
...
CREATE TABLE IF NOT EXISTS stock_receipt_voucher_items (
    id SERIAL PRIMARY KEY,
    receipt_voucher_id VARCHAR(256) NOT NULL REFERENCES stock_receipt_vouchers(id) ON DELETE CASCADE,
    product_id VARCHAR(256) NOT NULL REFERENCES products(id),
...
-- Renaming old GRN tables to be consistent
ALTER TABLE IF EXISTS goods_received_notes RENAME TO stock_receipt_vouchers_old;
ALTER TABLE IF EXISTS goods_received_note_items RENAME TO stock_receipt_voucher_items_old;

-- Note: The following tables were added for better inventory management, but the UI might still reference the old mock data.
-- This schema represents the final intended structure.
...
-- This script sets up the structure. You will need to use the application UI
-- or separate INSERT scripts to populate the tables with initial data like
-- default users, roles, chart of accounts, etc.
